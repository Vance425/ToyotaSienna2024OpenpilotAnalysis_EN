#!/usr/bin/env python3
"""
Focused shaping experiment for the city transition/settle replay regime.

This script intentionally leaves the main replay harness untouched and tests
only a small family of negative-tail slew boosts against three windows:
...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "analysis-output" / "transition_settle_shaping_experiment"
...
VARIANTS = [
    {"name": "baseline", "slew": 175.0, "neg_threshold": None, "neg_boost": 1.0},
    {"name": "neg_tail_boost_1p25", "slew": 175.0, "neg_threshold": -1000.0, "neg_boost": 1.25},
    {"name": "neg_tail_boost_1p50", "slew": 175.0, "neg_threshold": -1000.0, "neg_boost": 1.50},
    {"name": "neg_tail_boost_2p00", "slew": 175.0, "neg_threshold": -1000.0, "neg_boost": 2.00},
]


def apply_variant_slew(series: list[float], base_slew: float, neg_threshold: float | None, neg_boost: float) -> list[float]:
    if not series:
        return []
    out = [series[0]]
    current = series[0]
    for target in series[1:]:
        diff = target - current
        effective = base_slew
        if neg_threshold is not None and diff < 0 and target <= neg_threshold:
...
def mean_abs(values: list[float]) -> float:
    return mean(abs(v) for v in values) if values else 0.0
